...
var q = new Queue({ concurrency: 3, timeout: 10000 });
...
var companies = fsq.response.venues;
...
job.on('progress',function(data){
    pubnub.publish({ channel : "dnbRescue.progress", message : data })
});

job.on('newRecord',function(record){
    pubnub.publish({ channel : "dnbRescue.newRecord", message : record })
});

job.on("complete",function(){
    pubnub.publish({ channel : "dnbRescue.complete", message : "completed" });
});
